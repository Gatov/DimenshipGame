...
Expected: `Failed: 0`. Total is **35** tests — 27 in `Dimenship.Core.Tests` (1 pre-existing `GameInfoTests`, 10 `UnitsTests`, 16 `SimulationEngineTests`) and 8 `LayoutSerializerTests` in `Dimenship.Shell.Tests`. If the count differs, a test was lost; find it before continuing.
...
> falsifiable). 35 is the real number.
...
5. Draw sitting at 8.000 MW, rising to 10.000 MW on the ticks the smelter runs, and a rising `CAP HITS` count.
