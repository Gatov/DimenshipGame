...
public sealed partial class ShellRoot : Control
{
    ...
    {
        // Focus views. Overview is real; the rest state what they will become.
        _registry.Register(
...
`ShellRoot` references `OverviewFocus`, `EnergyBudgetPanel` and `EventLogPanel`, which Task 6 creates. **The solution will not build until Task 6 is done** — that is why this task's build check comes after them, and why the commit at Step 8 is the only one in the plan that lands non-compiling code. If that is unacceptable in your workflow, do Task 6 first and commit both together.

- [ ] **Step 7: Verify the Shell library types resolve**

```bash
dotnet build src/Dimenship.Shell/Dimenship.Shell.csproj
```

Expected: `Build succeeded`. This is the only build check available until Task 6 lands the three real panels.
...
Expected: `Build succeeded`, 0 errors, five projects. This is the first full build since Task 5 introduced forward references to these three panels.
...
```bash
git add dimenship/scripts/ui/panels && git commit -m "feat: add overview, energy budget, and event log panels"
```
...
Expected: `Failed: 0`. Total is 27 tests — 2 `GameInfoTests`, 10 `UnitsTests`, 13 `SimulationEngineTests`, 8 `LayoutSerializerTests`. If the count differs, a test was lost; find it before continuing.
