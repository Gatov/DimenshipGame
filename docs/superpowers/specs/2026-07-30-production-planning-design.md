...
- **Facility upgrades.** Work rate and energy efficiency are per-executor constants. The Schematics spec anticipates upgrades changing them without modifying schematics — the shape allows it, nothing implements it. `docs/superpowers/specs/2026-08-20-recycling-refit-and-construction-design.md` now settles *how* they will change: a fitted module is an item in a socket storage, and a machine reads its effective rates from what its sockets hold. That design adds no task type and no schematic field, so nothing in this spec is superseded by it.
